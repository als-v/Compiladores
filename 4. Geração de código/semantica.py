...
from arvore import buscarNos, alteracoesArvore
...
def main(file, detailed = False, showTree = False, showTable = False):
    ...
    if success:
        ...
        if None != tree:
            ...
            alteracoesArvore(tree)

            # salvar a arvore
            UniqueDotExporter(tree).to_picture(str(file) + '.poda.ast.png')
            
            if showTree:
                print('\nArvore sintatica podada:\n')
                print(RenderTree(tree, style=AsciiStyle()).by_attr())
            
            print('\nPoda na arvore gerada com sucesso!\nArquivo: ' + file + '.poda.ast.png')
            
            return tree, listaFuncoes, listaVariaveis, listaErros
