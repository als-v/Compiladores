...
def main(file, dLogs, sTree):
    ...
    detailedLogs,  showTree = dLogs, sTree
    ...
    if root and root.children != ():
        ...
        if showTree:
            print('\nArvore Sintática:')
            DotExporter(root).to_picture(file + '.ast.png')
            UniqueDotExporter(root).to_picture(file + '.unique.ast.png')
            DotExporter(root).to_dotfile(file + '.ast.dot')
            UniqueDotExporter(root).to_dotfile(file + '.unique.ast.dot')
            print(RenderTree(root, style=AsciiStyle()).by_attr())
            print('\nArvore gerada com sucesso.\nArquivo: ' + file + '.ast.png')
            ...
            DotExporter(root, graph="graph",
                        ...
                        edgetypefunc=MyNode.edgetypefunc).to_picture(file + ".ast2.png")
